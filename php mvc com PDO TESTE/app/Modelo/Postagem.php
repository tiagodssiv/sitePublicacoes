...
$sql="SELECT postagem.conteudo_sem_caractere,quantidade,id,arquivo,conteudo,destaque , titulo,date_format( data,'%d/%m/%Y às %h:%m' ) AS data FROM postagem WHERE month( data ) = month( now( ) )
...
$sql="SELECT conteudo_sem_caractere,quantidade,id,arquivo,destaque , titulo,date_format( data,'%d/%m/%Y às %h:%m' ) AS data FROM postagem WHERE month( data ) = month( now( ) )
AND year( data ) = year( now( ) )AND ex ='0' ORDER BY id DESC";
...
// se o campo check box não for preenchido recebe esse valor
 ...
 $con=Connection::getCon();
 $sql=$con->prepare('INSERT INTO postagem (titulo,conteudo,data,arquivo,id_categoria,destaque,conteudo_sem_caractere) values(:tit,:cont,:dat,:arq,:id_cat,:dest,:con)');
 ...
 $sql->bindValue(':id_cat',$dadosPost['id_categoria']);
  ...
  $sql->bindValue(':con',$dadosPost['stv']);
 $res=$sql->execute();
if($res==0){
