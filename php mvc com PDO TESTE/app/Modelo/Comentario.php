...
public static function selecionarComentario($post){
	...
	public function editarComentario($post)
	 {
		  $id=$post['id_comentario'];
		 $msg=$post['msg'];
		 $editado='Editado';
	     $conex= Connection::getCon();
		 $sql="UPDATE comentarios SET mensagem = ? , editado=? WHERE id_comentario = ?";
		 $sql=$conex->prepare($sql);
		 $res=$sql->execute(array($msg,$editado,$id));
		 if($res==0){
   ...
   public static function selecionarTodosComentarios()
	   ...
	   public static function selecionarComentariosRespostas()
    {
      ...
      $conn =new  PDO('mysql:host=127.0.0.1:3312; dbname=serie-criando-site;','root','');
        ...
        $sql = "SELECT * FROM usuario LEFT JOIN  respostasComentario ON id_user=id_usuario  LEFT JOIN comentarios  ON id_comentarios=id_comentario INNER JOIN postagem ON id_postagem=id ORDER BY id_resposta DESC";
        $sql =  $conn ->prepare($sql);
        $sql->execute();
...
$con=Connection::getCon();
$sql="INSERT INTO comentarios (nome , mensagem , id_postagem , data_hora ,numero,id_use)VALUES(:nome,:mensagem,:id_postagem,:data_hora,:numero,:id_use)";
...
$sql->bindValue(':numero',$rand);
$sql->bindValue(':id_use',$dados['id_usr']);
$res=$sql->execute();
